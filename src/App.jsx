    ...
    <div className="border border-zinc-700 rounded-2xl mb-3 overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 glass-header" style={{borderColor: TM_BLUE}}>
        <div className="flex items-center gap-2">
          <button onClick={() => setOpen(o=>!o)} className="w-6 h-6 border border-white rounded text-xs hover:bg-white hover:text-black">{open ? "−" : "+"}</button>
          <div>
...
export default function TournamentMaker() {
  ...
  const [targetTournamentId, setTargetTournamentId] = useState(NEW_TOURNEY_SENTINEL); // dropdown selection
  ...
  useEffect(() => {
    ...
    if (stored) {
      try { const data = JSON.parse(stored); if (Array.isArray(data)) setTournaments(data); } catch {}
    }
  }, []);
  ...
  function generateRound1Matches(teams, seedTopName, seedBottomName) {
    ...
    slots[0]=seedTopName; slots[size-1]=seedBottomName;
    ...
    const half=size/2; const topAvail=[]; const botAvail=[];
    for(let i=0;i<half;i++){ if(i!==0 && i!==1) topAvail.push(i); }
    for(let i=half;i<size;i++){ if(i!==(size-1) && i!==(size-2)) botAvail.push(i); }
    const order=[]; const L=Math.max(topAvail.length, botAvail.length);
    for(let i=0;i<L;i++){ if(i<topAvail.length) order.push(topAvail[i]); if(i<botAvail.length) order.push(botAvail[i]); }
    order.push(1, size-2);
  ...
  function createTournament() {
    ...
    if (!seed1 || !seed2 || seed1===seed2) { alert("Pick two different seeds."); return; }

    const nameIndex = Object.fromEntries(builderTeams.map(tm=>[tm.name, true]));
    if (!nameIndex[seed1] || !nameIndex[seed2]) { alert("Seeds must be from the added entries."); return; }
    ...
    // reset builder
    setTName(""); setNamesText(""); setSeed1(""); setSeed2(""); setBuilderTeams([]);
  ...
  function roundCounts(tn) {
    const mp = new Map();
    for (const m of tn.matches) { if (!(m.aId||m.bId)) continue; mp.set(m.round, (mp.get(m.round)||0)+1); }
    return mp;
  }
  ...
  function applyEntriesToTournament(tournamentId, newNames){
    ...
    setTournaments(prev => prev.map(tn => {
      ...
      // === 2) Any remaining names → collect new Round 1 matches (we'll insert them between top/bottom later) ===
      const newR1Matches = [];
        ...
        if (topIdx !== -1 && bottomIdx !== -1) {
          ...
          newR1Matches.forEach((nm, idx) => {
            if (idx % 2 === 0) {
              ...
              const pos = between.length - backInserts; // near bottom, moving inward
              between.splice(pos, 0, nm);
              backInserts++;
            }
          });
  ...
  // ----------------------------- Render -----------------------------
  // Hero header animations & 3D text styles
  const gpStyles = `
...
(function runDevTests(){
  try{
    const IS_DEV = true; // toggle to disable logs
    if(!IS_DEV) return;

    function assertEqual(name, got, expected){
      const pass = Array.isArray(expected) ? (JSON.stringify(got)===JSON.stringify(expected)) : (got===expected);
